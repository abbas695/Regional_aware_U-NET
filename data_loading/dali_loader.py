...
def random_augmentation(probability, augmented, original):
    condition = fn.cast(fn.random.coin_flip(probability=probability,seed=1), dtype=types.DALIDataType.BOOL)
...
class GenericPipeline(Pipeline):
    ...
    def load_data(self):
        ...
        if self.load_to_gpu:
            img = img.gpu()
        img = fn.reshape(img, layout="CDHW",seed=self.kwargs["seed"])
        if self.input_y is not None:
            lbl = self.input_y(name="ReaderY")
            if self.load_to_gpu:
                lbl = lbl.gpu()
            lbl = fn.reshape(lbl, layout="CDHW",seed=self.kwargs["seed"])
            return img, lbl
        return img
    ...
    def crop(self, data):
        return fn.crop(data, crop=self.patch_size, out_of_bounds_policy="pad",seed=self.kwargs["seed"])
    ...
    def transpose_fn(self, img, lbl):
        img, lbl = fn.transpose(img, perm=(1, 0, 2, 3),seed=self.kwargs["seed"]), fn.transpose(lbl, perm=(1, 0, 2, 3),seed=self.kwargs["seed"])
        return img, lbl
...
class TrainPipeline(GenericPipeline):
    ...
    @staticmethod
    def slice_fn(self,img):
        return fn.slice(img, 1, 3, axes=[0],seed=self.kwargs["seed"])
    ...
    def biased_crop_fn(self, img, label):
        ...
        anchor = fn.roi_random_crop(label, roi_start=roi_start, roi_end=roi_end, crop_shape=[1, *self.patch_size],seed=self.kwargs["seed"])
        anchor = fn.slice(anchor, 1, 3, axes=[0],seed=self.kwargs["seed"])  # drop channels from anchor
        img, label = fn.slice(
            [img, label], anchor, self.crop_shape, axis_names="DHW", out_of_bounds_policy="pad", device="cpu",seed=self.kwargs["seed"]
        )
        return img.gpu(), label.gpu()

    def zoom_fn(self, img, lbl):
        scale = random_augmentation(0.15, fn.random.uniform(range=(0.7, 1.0),seed=self.kwargs["seed"]), 1.0)
        d, h, w = [scale * x for x in self.patch_size]
        if self.dim == 2:
            d = self.patch_size[0]
        img, lbl = fn.crop(img, crop_h=h, crop_w=w, crop_d=d,seed=self.kwargs["seed"]), fn.crop(lbl, crop_h=h, crop_w=w, crop_d=d,seed=self.kwargs["seed"])
    ...
    def noise_fn(self, img):
        img_noised = img + fn.random.normal(img, stddev=fn.random.uniform(seed=self.kwargs["seed"],range=(0.0, 0.33)),seed=self.kwargs["seed"])
        return random_augmentation(0.15, img_noised, img)

    def blur_fn(self, img):
        img_blurred = fn.gaussian_blur(img, sigma=fn.random.uniform(range=(0.5, 1.5),seed=self.kwargs["seed"]),seed=self.kwargs["seed"])
        return random_augmentation(0.15, img_blurred, img)

    def brightness_fn(self, img):
        brightness_scale = random_augmentation(0.15, fn.random.uniform(range=(0.7, 1.3),seed=self.kwargs["seed"]), 1.0)
        return img * brightness_scale

    def contrast_fn(self, img):
        scale = random_augmentation(0.15, fn.random.uniform(range=(0.65, 1.5),seed=self.kwargs["seed"]), 1.0)
        return math.clamp(img * scale, fn.reductions.min(img,seed=self.kwargs["seed"]), fn.reductions.max(img,seed=self.kwargs["seed"]))

    def flips_fn(self, img, lbl):
        kwargs = {
            "horizontal": fn.random.coin_flip(probability=0.5,seed=self.kwargs["seed"]),
            "vertical": fn.random.coin_flip(probability=0.5,seed=self.kwargs["seed"]),
        }
        if self.dim == 3:
            kwargs.update({"depthwise": fn.random.coin_flip(probability=0.5,seed=self.kwargs["seed"])})
        return fn.flip(img, **kwargs), fn.flip(lbl, **kwargs,seed=self.kwargs["seed"])
