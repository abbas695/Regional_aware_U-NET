...
class CrossAttentionBlock3D(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0):
        super(CrossAttentionBlock3D, self).__init__()
    ...
    def forward(self, query, key, value):
        ...
        # Reshape to (L, N, E) for multi-head attention
        query = query.view(B, C, -1).permute(2, 0, 1)  # (L, N, E)
        key = key.view(B, C, -1).permute(2, 0, 1)      # (L, N, E)
        value = value.view(B, C, -1).permute(2, 0, 1)  # (L, N, E)
        print(query.shape)
        print(key.shape)
        print(value.shape)
        ...
        attn_output = self.norm1(attn_output[0] + query)
        
        # Apply fully connected layer and residual connection
        fc_output = self.fc(attn_output)
        fc_output = self.norm2(fc_output + attn_output)
        
        # Reshape back to (B, C, D, H, W)
        fc_output = fc_output.permute(1, 2, 0).view(B, C, D, H, W)
...
class UNet3D(nn.Module):
    def __init__(
        ...
        self.guidance_input_block_ed=InputBlock(2, self.filters[0], norm=self.norm)
        self.guidance_input_block_nec=InputBlock(2, self.filters[0], norm=self.norm)
        self.guidance_input_block_et=InputBlock(1, self.filters[0], norm=self.norm)
