...
from nnunet.vit import ViT
import matplotlib.pyplot as plt
from nnunet.CBAM import CBAM3D
normalizations = {
...
def attention_display(attention_map,name):
    plt.imshow(attention_map, cmap='hot', interpolation='nearest')
    plt.colorbar()
    plt.title("Attention Heat Map Using Mean for Slice Index 64"+name)
    name="Attention Heat Map Using Mean for Slice Index 64"+name
    # Save the figure as an image file
    plt.savefig(f"Architecture/{name}", format="png", dpi=300)
    plt.show()
...
class UpsampleBlock_withguidance(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, **kwargs):
        ...
        self.conv_with_guidance1 = ConvBlock1(out_channels*2 + in_channels, out_channels, kernel_size, 1, **kwargs)
        self.conv_with_guidance2 = ConvBlock2(out_channels, out_channels, kernel_size, 1, **kwargs)


        self.SE=SqueezeExcitation(224,8)


    def forward(self, x, x_skip, guidance):
        x = nn.functional.interpolate(x, scale_factor=2, mode="trilinear", align_corners=True)

        x = torch.cat((x, x_skip,guidance), dim=1)
        x=self.SE(x)
...
class UNet3D(nn.Module):
    def __init__(
        ...
        self.input_block = InputBlock(3, self.filters[0], norm=self.norm)
        self.guidance_input_block_ed=InputBlock(1, self.filters[0], norm=self.norm)
        self.guidance_input_block_nec=InputBlock(1, self.filters[0], norm=self.norm)
        self.guidance_input_block_et=InputBlock(2, self.filters[0], norm=self.norm)
    ...
    def forward(self, input_data):
        ...
        out_guidance_ed=self.guidance_input_block_ed(input_data[:,0:1,:,:,:])
      
        out_guidance_nec=self.guidance_input_block_nec(input_data[:,1:2,:,:,:])
        out_guidance_et=self.guidance_input_block_et(torch.cat((input_data[:,1:2,:,:,:], input_data[:,2:3,:,:,:]), dim=1))
        encoder_outputs = [out]
        ...
        ET_decoder_outputs.append(ET_out)
        # Assuming 'conv_output' is your 128x128x128x64 tensor
